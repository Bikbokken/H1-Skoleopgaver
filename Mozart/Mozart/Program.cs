...
for (int i = 0; i < menuets.GetLength(0); i++)
{
    ...
    {
        Console.WriteLine(menuets[i, j]);
        files.Add(@"C:\Users\Bruger\Desktop\H1-Skoleopgaver\Mozart\Mozart\Wave\M" + menuets[i, j] + ".wav");
    }
}

for (int i = 0; i < trios.GetLength(0); i++) { 
    for (int j = 0; j < trios.GetLength(1); j++)
    {
        Console.WriteLine(trios[i, j]);
        files.Add(@"C:\Users\Bruger\Desktop\H1-Skoleopgaver\Mozart\Mozart\Wave\T" + trios[i, j] + ".wav");
    }
}
